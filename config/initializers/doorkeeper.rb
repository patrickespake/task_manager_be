...
Doorkeeper.configure do
  ...
  resource_owner_from_credentials do |_routes|
    found_user = User.find_for_authentication(email: params[:email])

    if found_user
      User.authenticate(found_user, params[:password]) ? found_user : nil
    end
  end
...
module CustomTokenErrorResponse
  def body
    {
      errors: [
        { detail: ::ResponseCode::CODES[:authentication]&.dig(self.name) }
      ]
    }
  end
end
